...
if "__main__" == __name__:
    ...
    parser.add_argument(
        "--split_csv",
        type=str,
        default="metadata_images_split_scene_v1.csv",
    )
    ...
    for split in ["train", "val", "test"]:
        ...
        for i, row in tqdm(split_meta_df.iterrows(), total=len(split_meta_df)):
            ...
            with h5py.File(os.path.join(dataset_dir, normal_path)) as f:
            ...
            normal = (normal + 1) / 2.0 * 255.0
            normal = normal.astype(np.uint8)
            normal_name = f"normal_{row.camera_name}_fr{row.frame_id:04d}.png"
            normal_path = os.path.join(scene_path, normal_name)
            cv2.imwrite(
                os.path.join(split_output_dir, normal_path),
            ...
            split_meta_df.at[i, "normal_path"] = normal_path
        ...
        with open(
            ...
            lines = split_meta_df.apply(
                lambda r: f"{r['rgb_path']} {r['depth_path']} {r['normal_path']}", axis=1
            ).tolist()
            f.writelines("\n".join(lines))
